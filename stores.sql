...
CASE 
    ...
    FROM data_store.orders;
#Query the table for the Order_ID with the highest sales
SELECT Order_ID, Sales, Profit FROM data_store.orders
WHERE Sales= (SELECT MAX(Sales) FROM data_store.orders) ;
